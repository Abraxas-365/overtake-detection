...
impl LaneLegalityDetector {
    ...
    pub fn estimate_ego_lane_boundaries(
        ...
        // Get all road markings with low threshold to catch faint lines
        let markings = self.get_markings_only(frame, width, height, 0.20)?;
        ...
        // Filter for lane boundary lines only (exclude arrows, crosswalks, etc.)
        let lane_lines: Vec<_> = markings
            .iter()
            .filter(|m| {
                // Class IDs: 4-10 = lane lines, 99 = mixed double
                matches!(m.class_id, 4 | 5 | 6 | 7 | 8 | 9 | 10 | 99)
            })
            .collect();
        ...
        // Find closest lane line on left and right side of vehicle
        let mut left_candidate: Option<(&DetectedRoadMarking, f32)> = None;
        let mut right_candidate: Option<(&DetectedRoadMarking, f32)> = None;
        ...
        for marking in &lane_lines {
            ...
            // Only consider lines that are reasonably close (within frame thirds)
            if distance > width as f32 / 3.0 {
                continue;
            }
        ...
        // Need both boundaries to define a valid ego lane
        if let (Some((left, _)), Some((right, _))) = (left_candidate, right_candidate) {
            let left_x = (left.bbox[0] + left.bbox[2]) / 2.0;
            let right_x = (right.bbox[0] + right.bbox[2]) / 2.0;

            // Sanity check: lane width should be reasonable
            let lane_width = right_x - left_x;
            if lane_width < 200.0 || lane_width > 800.0 {
                debug!(
                    "⚠️  YOLOv8-seg: Rejected lane width {:.1}px (expected 200-800px)",
                    lane_width
                );
                return Ok(None);
            }

            // Use average confidence of both boundaries
            let avg_confidence = (left.confidence + right.confidence) / 2.0;

            debug!(
                "✅ YOLOv8-seg lanes: L={:.1}px ({}), R={:.1}px ({}), width={:.1}px, conf={:.2}",
                left_x, left.class_name, right_x, right.class_name, lane_width, avg_confidence
            );

            Ok(Some((left_x, right_x, avg_confidence)))
        } else {
            // Only found one boundary or none
            debug!(
                "⚠️  YOLOv8-seg: Incomplete lane (left: {}, right: {})",
                left_candidate.is_some(),
                right_candidate.is_some()
            );
            Ok(None)
        }
    }
