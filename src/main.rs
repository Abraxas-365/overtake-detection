...
use types::{Lane as AnalysisLane, LaneChangeConfig, LaneChangeEvent, Point as AnalysisPoint};
...
async fn process_video(
    ...
    // Initialize Python-style lane change analyzer
    ...
    while let Some(frame) = reader.read_frame()? {
        frame_count += 1;
        let timestamp_ms = frame.timestamp * 1000.0;
        ...
        match process_frame(&frame, inference_engine, config).await {
            Ok(lanes) => {
                ...
                let analysis_lanes: Vec<AnalysisLane> = lanes
                    .iter()
                    .enumerate()
                    .map(|(i, lane)| AnalysisLane {
                        lane_id: i,
                        points: lane
                            .points
                            .iter()
                            .map(|p| AnalysisPoint::new(p.0, p.1))
                            .collect(),
...
fn save_results(
    ...
    // Save as JSON Lines (like Python)
    let jsonl_path =
